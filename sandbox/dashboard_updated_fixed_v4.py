...
# Load incoming (don't stop the whole app: other tabs should still work)
incoming_df = pd.DataFrame(columns=KDD_COLUMNS)
incoming_ready = True
...
if incoming_ready and incoming_df.empty:
    # Keep the rest of the app usable even if the feed is empty
...
def render_live(bypass_active, top_n, auto_refresh, refresh_sec):
    ...
    for i in range(min(top_n, len(triage_view))):
        ...
        with st.container(border=True):
            ...
            with right:
                    ...
                    # Only allow actions for HIGH / CRITICAL (understaff guardrail)
                    allow_actions = sev in ("CRITICAL", "HIGH")
                    if not allow_actions:
                        st.caption("ℹ️ Actions are enabled only for **HIGH** and **CRITICAL** alerts.")
                    ...
                    with c2:
                        ...
                        bypass_disabled = (not allow_actions) or reason_missing
                        if reason_missing and allow_actions:
    ...
    # -----------------------------
    # st.info("Live Events (Recent) moved to its own tab: **📊 Live Events (Recent)**.")
